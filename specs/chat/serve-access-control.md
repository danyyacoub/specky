...
5. **Login for a deployed server** — set `SPECKY_AUTH_USERNAME` and `SPECKY_AUTH_PASSWORD` in the server's environment and every route — pages, stylesheets, `/chat`, `/search` — answers 401 with a `WWW-Authenticate: Basic` challenge until the request carries those credentials. The browser shows its own login prompt and then re-sends the credentials on every same-origin load, which is why, unlike the token, this can gate static files. Only CORS preflights skip it, because browsers never attach credentials to one. The credentials live in the environment only, never in `specky.toml`: that file is committed, and a password in it would ship with the repo it is meant to protect. Setting only one of the two variables stops `serve` from starting rather than serving the repo open.
