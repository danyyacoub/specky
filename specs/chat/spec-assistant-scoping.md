...
1. **Type `@` in the chat input** — a dropdown appears listing available modules and features. The picker only opens on an `@` that starts a word, so typing an email address doesn't hijack Enter.
