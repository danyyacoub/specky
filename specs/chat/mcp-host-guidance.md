...
2. **Receive instructions**: the server sends its connect-time instructions, telling the model to check specky first for what a feature does, how a flow works or why it changed, to cite the doc, and to verify against the doc's `sources` before changing code on its word.
...
| `search_docs` says nothing is indexed | The skill runs `specky index` (no AI call) and searches again. | Search needs an index to have anything to find. |
...
| `search_docs` says nothing is indexed | It is caught | The skill runs `specky index` (no AI call) and searches again. |
