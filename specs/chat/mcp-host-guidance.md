...
| Question is what a feature is meant to do before it is used | `find-feature` answers from the doc and cites the doc path and behaviour ids; `explore-docs` keeps the broader how and why questions. | The lookup is the same either way, and the narrower skill is short enough for a cheap model to run. |
...
| The lookup skills are available to the model | A behaviour question arrives | The model follows the skill's lookup: search, read, the doc's behaviour ids, its history, and the sources check. |
| A question asks what a feature is meant to do before it is used | `find-feature` is consulted | The answer leads with the intended use and cites the doc path and behaviour ids. |
