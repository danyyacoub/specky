...
stale_after_days = 14             # how far a doc may lag its code before it's reported as stale
```
...
| A file has several covering docs and the range updated one of them | Clean — any one is enough |
...
| Multiple docs, one updated | A file described by two docs; the range changes it and one of them | Run `specky check` | No violation; exit 0 |
| Another domain's doc | The range changes covered code and updates a doc in an unrelated domain | Run `specky check` | Violation |
| Stale doc is advice | A doc already behind its code; the range documents itself elsewhere in the domain | Run `specky check` | The stale doc is listed; no violation; exit 0 |
| Stale doc the range fixed | The same doc, updated by this range | Run `specky check` | Not reported as stale |
| Stale docs elsewhere | A stale doc covering no file in this range | Run `specky check` | Counted, not listed |
| Owner missing | A covered doc with no `owner:` | Run `specky check` | The doc is named with `no \`owner:\``; no violation; exit 0 |
| Owner present | The same doc with `owner: Payments team` | Run `specky check` | Not mentioned |
| History docs never asked for an owner | Any range with `specs/history/` docs | Run `specky check` | None of them appear as unowned |
| Unowned doc outside the range | An unowned doc covering nothing this range touched | Run `specky check` | Not mentioned |
