...
`specky check` is the CI gate that fails if a pull request changes code that a doc describes without updating that doc. It costs nothing to run — no AI provider, no history walk — and answers from the diff and the `doc_files` table `specky index` derives from git. It fails by default; use `--advisory` to print the report and exit 0, which is how a repo adopts the gate before it's clean.
...
5. **Check for violations.** A violation is a covering doc whose whole domain the range left untouched; pairs must recur across `[check] min_link_commits` (default 2) separate commits to fail.
...
| Code changed, covering doc untouched, pair seen ≥`min_link_commits` commits | Violation; exit 1 |
...
| Undocumented file | Changed file no doc covers | Run `specky check` | Warning; 0% coverage; exit 0 |
