...
2. **Mermaid diagrams render to SVG** — `vendor/mermaid-render/` (Node + beautiful-mermaid) converts each diagram to static `<svg>` at `render-html` time; if setup missing, diagram source stays as plain text, render completes anyway.
