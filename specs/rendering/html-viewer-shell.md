...
1. **Navigation Structure** — Sidebar organizes docs by domain in collapsible groups, with domain icons and an active indicator on the current page.
...
4. **Visual Polish** — Titlebar, sidebar, chat panel, and search dropdown use glass/blur effects and semantic color tokens (light/dark variants chosen by system preference). Icons are a hand-built SVG sprite (no icon font or new dependency).
