...
5. **Check Devin login** — When the provider command is `devin` and the tool is on the path, `specky doctor` runs a local login-status check and reports whether Devin CLI is logged in.
