...
specky: Claude Code / opencode / Kiro / Devin plugin. Generates + maintains functional docs in place
...
Per-host wiring lives in [integrations/](integrations/) — Claude Code is the only plugin host of the
four, so the others get the MCP server + `document-domain` skill by hand. [AGENTS.md](AGENTS.md) is
this file's pointer for agents that read that convention instead (Devin among them); keep it a
...
```bash
uv run specky init               # configure AI provider, writes specky.toml (gitignored). Interactive
                                 # unless answered up front: --yes takes the defaults, --provider /
                                 # --model / --api-key-env / --base-url / --command / --docs-root
                                 # answer one question each, --no-validate skips the live test call.
                                 # A scripted setup (Dockerfile, CI, a cloud agent's VM) has no TTY
