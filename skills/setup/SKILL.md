...
Every step is a plain `specky` command, so a user can also run them by hand. Check what's already
...
- **No `specky` on PATH**: ask whether to install the CLI with `uv tool install specky`. Run it only
  if they say yes, then repeat the check. Explain why: the git hooks installed in step 4 call a
...
a `detail`. Skip step 3 if `config` already reports a provider, step 4 if all three `git hook`
entries are `ok`, and step 5 if the index is current. If everything is `ok`, say the repo is already
set up and go to the report.
...
- What was already set up and what this run changed: provider, hooks, index. Mention that
  `specky.toml` went into `.gitignore` if `init` said so.
