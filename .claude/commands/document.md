...
## Inputs
Ask the user for:
1. **Domain name** — which module/area to document (e.g., "accounts", "docs", "integrations", "ai", "shell")
...
- Functional docs live in **`specs/<domain>/`**, mirroring `app/domains/<domain>/` (e.g., `specs/docs/enrichment.md`, `specs/accounts/auth.md`). Cross-cutting docs stay at the `specs/` root.
- File name = a kebab-case slug of the **topic** (not `README.md`). Prefer the module name (e.g., `enrichment.md`) or a descriptive topic (e.g., `source-index.md`, `functionalities-catalog.md`).
- One `.md` per topic. If a module has multiple distinct topics, split into multiple files (e.g., `rag-search.md` + `persistence.md`).
- If the domain folder doesn't exist yet under `specs/`, create it.

## Steps

### 1. Discover the domain
- Find all related files: `find . -path "*{domain}*" -not -path "*/node_modules/*" -not -path "*/__pycache__/*" -not -path "*/.venv/*"`
- Identify backend modules (`apps/backend/app/domains/<domain>/`), frontend features (`apps/frontend/src/features/`), jobs, workflows, routers, services
...
- **`specs/PRODUCT.md`** — what Glia is and who it is for; the product framing every doc is written against.
...
- **`specs/documentation.md`** — the house style these docs follow.
...
### 5. Get the type of functionality, is this a feature, a whole workflow or a specific case
- In case of Workflow, identify the sequence of steps and the expected outcomes for each step.
- In case of a feature, identify the specific functionality and its expected outcomes.
- In case of a specific case, identify the unique scenario and its expected outcome.
...
- Each scenario is concrete: real-ish values, named documents/entries, expected outputs.
...
- Prefer scenarios that map to (or already have) real tests in `apps/backend/tests/` or `apps/frontend/src/` — link them when they exist.
...
Open `specs/MODULES.md` at the repo root and keep the index current. It is a set of `| Doc | Purpose |` tables — one section for `Cross-cutting`, then one per domain (`accounts`, `ai`, `docs`, `integrations`, `shell`), and the order of the modules is the order the Functionalities page lists them in.
...
- Cross-module relationships belong in `specs/architecture.md` (which carries the Mermaid diagrams), not in `MODULES.md` — if step 4 turned up a new dependency between domains, update that doc instead.
...
- Glia reads `MODULES.md` and `GLOSSARY.md` back out of any source it browses and treats them as the authority on that source's hierarchy and vocabulary, so both must stay accurate.
